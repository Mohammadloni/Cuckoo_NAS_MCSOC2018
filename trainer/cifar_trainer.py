...
class Cifar10Trainer(ClassifyTrainer):
    ...
    def __init__(self, batch_size=32, epochs=100, verbose=0, lr=None, model_path='/home/user1/loni/cgp-cnn-design-master3/tmp/', stats_path='/home/user1/loni/cgp-cnn-design-master3/tmp/'):
        """
        Parameters
        ----------
        batch_size: int(32)
    ...
    def __call__(self, model, epoch, initial_epoch=0):
        ...
        if params_factor <= 0.0 or flops_factor <= 0.0:
            print("-----------  first -------")
            return self.worst, bad_params_factor
        ...
        score = params_factor * 0.1 + acc * 0.9
        #score = acc
        # score = (params_factor + flops_factor + acc) / 3.0

        print("\n%s" % ("-" * 100))
        print("acc: %.4f ---> params: %d, %.4f ---> flops: %s, %.4f ---> score: %.4f" %
